...
const totalCost = (email, orderId) => {
  ...
  let order = shop.customers[email].orders.filter(order => order.id === orderId)[0]
  return order.items.reduce((accumulator, item) => accumulator + (productCost(item.productId) * item.qty), 0)
}
